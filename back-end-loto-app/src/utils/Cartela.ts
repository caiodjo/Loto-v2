...
export class Cartela {
  ...
  public filtrarNumero(numero: number): number[][] {
    const cartelasFiltradas = this.cartelas.filter((cartela) => {
      cartela.forEach((el) => {
  ...
  static createEvenOddCombinations(qtdPares: 6 | 7 | 8): number[][] {
    ...
    const evenComb: Combination<number> = new Combination(
      this.padroes.pares,
      qtdPares
    );
    const qtdImpares: number = 15 - qtdPares;
    const oddComb: Combination<number> = new Combination(
      this.padroes.impares,
      qtdImpares
    );
  ...
  /*
        ...
        return: allPossibilities -> concatenação de listas de numeros pares com lista de impares.
    */
  static concatenarPossibilidades(
    arr1: number[][],
    arr2: number[][],
    qtdPares: 6 | 7 | 8
  ): number[][] {
    ...
    switch (qtdPares) {
      ...
      case 7:
        return (
          ...
          (primos === 5 && (fibos === 4 || fibos === 5)) ||
          (primos === 6 &&
            (fibos === 3 || fibos === 4 || fibos === 5 || fibos === 6)) ||
          (primos === 7 && fibos === 4)
        );
      case 8:
        return (
